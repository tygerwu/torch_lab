...
template <bool UseCopyFence,
          typename TiledCopy,
          typename PredTensor,
          typename ThrSrcEngine,
          typename ThrSrcLayout,
          typename ThrDstEngine,
          typename ThrDstLayout>
CUTE_HOST_DEVICE void CopyIf2D(const TiledCopy&                          tiled_copy,
...
template <Pred2DAxis PredAxis,
          ...
          typename ThrDstEngine,
          typename ThrDstLayout>
CUTE_DEVICE void Copy2DWithMask(const TiledCopy&                          tiled_copy,
                                const Tensor<ThrSrcEngine, ThrSrcLayout>& thr_src,
                                Tensor<ThrDstEngine, ThrDstLayout>&       thr_dst,
                                int                                       slice_id,
                                int                                       m_beg,
...
template <Pred2DAxis PredAxis,
          bool       UseCopyFence,
          typename TiledCopy,
          typename SrcMemEngine,
          typename SrcMemLayout,
          typename DstMemEngine,
          typename DstMemLayout>
CUTE_DEVICE void SMem2DCopyWithMasK(const TiledCopy&                          tiled_copy,
    ...
    Copy2DWithMask<PredAxis, SM, SN, UseCopyFence>(tiled_copy, s2g_src, s2g_dst, slice_id, m_beg, m_end, n_beg, n_end);
}
...
template <Pred2DAxis PredAxis,
          ...
          typename RMemEngine,
          typename RMemLayout>
CUTE_DEVICE void G2RCopy2DWithMasK(const TiledCopy&                      tiled_copy,
                                   const Tensor<GMemEngine, GMemLayout>& gmem_block,
                                   Tensor<RMemEngine, RMemLayout>&       reg,
                                   int                                   slice_id,
                                   int                                   m_beg,
                                   int                                   m_end,
                                   int                                   n_beg,
                                   int                                   n_end) {
    static_assert(rank(GMemLayout{}) == rank(RMemLayout{}));

    // SrcMemLayout:(GM,GN,~,~)
    constexpr int GM = size<0>(GMemLayout{});
    constexpr int GN = size<1>(GMemLayout{});

    auto thr_copy = tiled_copy.get_slice(slice_id);

    auto g2r_src = thr_copy.partition_S(gmem_block);  // (CopyAtomSize),G2R_ValTile_M,G2R_ValTile_N
    auto g2r_dst = thr_copy.retile_D(reg);
    Copy2DWithMask<PredAxis, GM, GN, false>(tiled_copy, g2r_src, g2r_dst, slice_id, m_beg, m_end, n_beg, n_end);
}
...
template <Pred2DAxis PredAxis,
          ...
          typename RMemEngine,
          typename RMemLayout>
CUTE_DEVICE void R2GCopy2DWithMasK(const TiledCopy&                      tiled_copy,
                                   const Tensor<RMemEngine, RMemLayout>& reg,
                                   Tensor<GMemEngine, GMemLayout>&       gmem_block,
                                   int                                   slice_id,
                                   int                                   m_beg,
                                   int                                   m_end,
                                   int                                   n_beg,
    ...
    auto r2g_src = thr_copy.retile_S(reg);  // (CopyAtomSize),G2R_ValTile_M,G2R_ValTile_N
    auto r2g_dst = thr_copy.partition_D(gmem_block);
    Copy2DWithMask<PredAxis, GM, GN, false>(tiled_copy, r2g_src, r2g_dst, slice_id, m_beg, m_end, n_beg, n_end);
}
