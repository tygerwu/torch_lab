...
struct SMemLayoutAtomTrait<1, 32, LogicalLayoutAtom> {
    using SMemLayoutAtom = decltype(composition(Swizzle<1, 4, 3>{}, LogicalLayoutAtom{}));
};

template <typename LogicalLayoutAtom>
struct SMemLayoutAtomTrait<1, 64, LogicalLayoutAtom> {
    using SMemLayoutAtom = decltype(composition(Swizzle<2, 4, 3>{}, LogicalLayoutAtom{}));
};

template <typename LogicalLayoutAtom>
struct SMemLayoutAtomTrait<1, 128, LogicalLayoutAtom> {
    using SMemLayoutAtom = decltype(composition(Swizzle<3, 4, 3>{}, LogicalLayoutAtom{}));
};

// ==============================Swizzle for QuadByte

template <typename LogicalLayoutAtom>
struct SMemLayoutAtomTrait<4, 16, LogicalLayoutAtom> {
    using SMemLayoutAtom = decltype(composition(Swizzle<2, 2, 3>{}, LogicalLayoutAtom{}));
};

template <typename LogicalLayoutAtom>
struct SMemLayoutAtomTrait<4, 32, LogicalLayoutAtom> {
    using SMemLayoutAtom = decltype(composition(Swizzle<3, 2, 3>{}, LogicalLayoutAtom{}));
};
...
struct MMajorConfig {
    ...
    using Copy = decltype(make_tiled_copy(Copy_Atom<UniversalCopy<VecType>, T>{}, ThrLayout{}, ValLayout{}));

    using AsyncCopyMeta =
        decltype(make_tiled_copy(Copy_Atom<SM80_CP_ASYNC_CACHEGLOBAL<VecType>, T>{}, ThrLayout{}, ValLayout{}));
